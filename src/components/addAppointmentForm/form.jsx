...
export default function({childToParent}) {
	...
	const formik = useFormik({
		...
		onSubmit: values => {
			axios.post(`https://todoapp-api-22b3.onrender.com/add-appointment/john_oliver`, values);
			childToParent(false);
		}
	});
	...
	return (
		<>
			<form id="wrapper" className="text-sm" onSubmit={formik.handleSubmit}>
				<h1 className="text-center font-bold text-bg-purple text-lg"> Add a New Stickie</h1>
				...
				<div className="flex gap-2 justify-center mt-2">
					<button type="reset" className="btn border-bg-purple">CLEAR</button>
					<button type="submit" className="btn bg-bg-purple text-white hover:border-xl border-bg-purple">ADD</button>
				</div>
			</form>
		</>
	)
